...
class InconstantOrdering:
    ...
    def get_min_string(self):
        ...
        def helper(block_index, prev_letter):
            ...
            if block_index % 2 == 0:
                def generate_sequence(start, length, current_seq):
                    if length == 0:
                        yield current_seq
                        return
                    ...
                    for c in range(start, 26):
                        if not current_seq or c > current_seq[-1]:
                            yield from generate_sequence(c + 1, length - 1, current_seq + (c,))
                    for seq in generate_sequence(prev_letter + 1, current_block_size, ()):
                        last_letter = seq[-1]
