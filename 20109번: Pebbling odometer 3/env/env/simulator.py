...
class Simulation:
    ...
    def __init__(self, grid_side=DEFAULT_GRID_SIDE,
                 ...
                 grid=None, posx=0, posy=0, dir=0, step_num=0, ip=0):
    ...
    def execute_runner(self):
        # Start the runner
        popen = subprocess.Popen(self.runfile, # type: ignore
                                 stdin=subprocess.PIPE,
                                 stdout=subprocess.PIPE, text=True) # type: ignore
        ...
        popen.stdin.write(("%d %d %d %d %d %d\n" % (self.posx, self.posy, self.dir, self.step_num, self.ip, max_steps_val))) # type: ignore
        for i in range(self.grid_side):
            for j in range(self.grid_side): popen.stdin.write(("%d " % (self.grid[i][j]))) # type: ignore
            popen.stdin.write(("\n")) # type: ignore
        popen.stdin.close() # type: ignore
        ...
        self.posx, self.posy, self.dir, self.step_num, self.ip, self.killed = \
            map(int, popen.stdout.readline().strip().split(' ')) # type: ignore
        self.killed = True if self.killed != 0 else False
        for i in range(self.grid_side):
            self.grid[i] = map(int, popen.stdout.readline().strip().split(' ')) # type: ignore
