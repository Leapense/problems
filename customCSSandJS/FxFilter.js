...
class FxFilter {
    ...
    static add(options) {
        ...
        if (typeof options === 'string') {
            ...
            fxConsole1.log(`🔧 Registered filter: "${name}"`);
        } else {
            // New format: add({name, callback, updatesOn})
            const { name, callback, updatesOn = [] } = options;
            this.filters.set(name, callback);
            this.filterOptions.set(name, { name, callback, updatesOn });
            fxConsole1.log(`🔧 Registered filter: "${name}" with updatesOn: [${updatesOn.join(', ')}]`);
        }
    }

    static init() {
        fxConsole1.log('🔄 FxFilter.init() called');
        ...
        if ('CSS' in window && 'registerProperty' in CSS) {
            try {
                ...
                fxConsole1.log('✅ --fx-filter property registered');
            } catch (e) {
                fxConsole1.log('⚠️ CSS registerProperty not supported or already registered');
            }
        }

        if (!this.running) {
            fxConsole1.log('🚀 Starting FxFilter with MutationObserver');
            this.running = true;
            ...
            this.waitForDOMReady(() => {
                ...
                fxConsole1.log('🎯 Initial DOM scan completed');
            });
        } else {
            fxConsole1.log('⚡ FxFilter already running - skipping duplicate initialization');
        }
    }
    ...
    static setupStyleWatcher() {
        ...
        fxConsole1.log('⏰ Style watcher setup complete (checking every 100ms)');
    }
    ...
    static setupResizeObserver() {
        if ('ResizeObserver' in window) {
                ...
                entries.forEach(entry => {
                    ...
                    if (this.pendingElements.has(element)) {
                        ...
                        if (hasValidDimensions) {
                            fxConsole1.log('📏 Element now has valid dimensions, processing:', element);
            ...
            fxConsole1.log('📏 ResizeObserver setup complete');
        } else {
            fxConsole1.log('⚠️ ResizeObserver not supported');
        }
    }
    ...
    static setupMutationObserver() {
        ...
        fxConsole1.log('👁️ MutationObserver setup complete');
    }
    ...
    static cleanupRemovedElement(element) {
        ...
        if (storedState && storedState.hasContainer) {
            ...
            fxConsole1.log('🧹 Cleaned up removed element from tracking');
        }
    ...
    static cleanup() {
        ...
        this.running = false;
        fxConsole1.log('🧹 FxFilter cleanup completed');
    }
        ...
        elements.forEach(element => {
            ...
            if (fxFilter) {
                ...
                if (hasZeroDimensions) {
                    ...
                    fxConsole1.log('⏳ Element has zero dimensions, adding to pending list:', element, {
                ...
                if (this.pendingElements.has(element)) {
                    this.pendingElements.delete(element);
                    fxConsole1.log('✅ Element now has dimensions, processing:', element);
                }
    ...
    static addFxContainer(element, filterValue, parsedFilter) {
        ...
        fxConsole1.log('Parsed filters:', { orderedFilters, customFilters });
        ...
        if (backdropFilter.trim()) {
            ...
            `);
            fxConsole1.log('Applied combined filter:', backdropFilter);
        } else {
            fxConsole1.log('No valid filters found');
        }
    }
    ...
    static updateFxFilter(element, filterValue, parsedFilter, filterId) {
        const { orderedFilters, customFilters } = parsedFilter || this.parseFilterValue(filterValue);
        fxConsole1.log('Updating filter:', { orderedFilters, customFilters });
        const filterParts = [];
        let svgContent = '';
        let customFilterIndex = 0;
        orderedFilters.forEach(item => {
        ...
        fxConsole1.log('Updated combined filter:', backdropFilter);
    }

    static createUnifiedSVG(customFilters) {
        fxConsole1.log('createUnifiedSVG called with:', customFilters);
        ...
        customFilters.forEach((filter, index) => {
            fxConsole1.log('Processing filter:', filter.name, 'with params:', filter.params);
            const callback = this.filters.get(filter.name);
            fxConsole1.log('Callback found:', !!callback);
            ...
            if (callback) {
                ...
                fxConsole1.log('Filter content generated:', filterContent);
                svgContent += `<filter id="${filterId}" x="-20%" y="-20%" width="140%" height="140%">${filterContent}</filter>`;
            }
        });

        fxConsole1.log('Final SVG content:', svgContent);
        svg.innerHTML = svgContent;
        fxConsole1.log('SVG element created:', svg);
    ...
    static parseFilterValue(filterValue) {
        ...
        fxConsole1.log('🔍 Parsing filter value:', filterValue);
        ...
        while ((match = filterRegex.exec(filterValue)) !== null) {
            ...
            fxConsole1.log(`📝 Found filter: ${filterName} with params: "${params}"`);

            if (this.filters.has(filterName)) {
                fxConsole1.log(`✅ Custom filter "${filterName}" found in registry`);
                ...
                fxConsole1.log(`📋 Parsed params for "${filterName}":`, paramArray);
                ...
                fxConsole1.log(`🎨 CSS filter: ${filterName}(${params})`);
                // It's a native CSS filter
                orderedFilters.push({ type: 'css', filter: `${filterName}(${params})` });
            }
        }

        fxConsole1.log('📊 Parse results:', { orderedFilters, customFilters });
        return {
    ...
    static stylesChanged(oldStyles, newStyles) {
        ...
        for (const [prop, value] of newStyles) {
            if (oldStyles.get(prop) !== value) {
                fxConsole1.log(`🔄 Style change detected: ${prop} changed from "${oldStyles.get(prop)}" to "${value}"`);
                return true;
            }
        }
